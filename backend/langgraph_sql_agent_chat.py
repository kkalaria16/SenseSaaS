...
try:
    ...
    DB_SERVER = os.getenv("DB_SERVER", "tcp:sql1234.site4now.net,1433")
    DB_DATABASE = os.getenv("DB_DATABASE", "db_sensesaas")
    DB_UID = os.getenv("DB_UID", "sensesaas_admin")
...
def get_sql_generation_prompt() -> ChatPromptTemplate:
    return ChatPromptTemplate.from_messages(
        ...
        - **User Intent:**
            - If the user asks for "Orders", they want the *Order Date*, *Total Amount*, and *Status*. They do NOT want the `OrderId` or `CompanyId`.
            - If the user asks for "Customers", they want the *Name* and *Email*, not the `CustomerId`.
        - **Readability:**
            - Prefer descriptive text columns over numeric codes.
            - Limit output to the most relevant 6-8 columns.
        - **ENUM/LOOKUP COLUMNS (CRITICAL):** Check the schema description for any column that defines a mapping (for example, "1=Pending, 2=Completed").
            - **FILTERING:** Use the **INTEGER** value (for example, `Status IN (1, 2)` if mapped, otherwise string).
            - **SELECTING:** Select the raw column (for example, `Status`). The system will automatically map it to the human-readable name.
        ...
        9. **ABSOLUTELY NO TECHNICAL/ID COLUMNS IN SELECT** unless the user explicitly asks for "ID", "Order Number", etc. For example, if they ask for "Orders", give them Order Date, Status, Total - NOT OrderId.
        ...
        - **Revenue**: Calculated as the sum of 'TotalAmount' from the 'Orders' table where 'Status' is 'Completed'.
        - **Customer Lifetime Value (LTV)**: Sum of 'TotalAmount' for all completed orders for a specific customer.
        - **Top Selling Products**: Determined by summing 'Quantity' in 'OrderItems' joined to 'Orders' (where Status = 'Completed') and grouped by Product.
        - **Order Status**: Common statuses are 'Pending', 'Completed', 'Shipped', 'Cancelled'.
        - **Inventory Management**: Check 'Inventory' table for 'QuantityOnHand' to see stock levels.
        - **Reviews**: Average rating is calculated from the 'Rating' column in the 'Reviews' table.
...
def table_selector(state: AgentState) -> AgentState:
    ...
    3. If the user asks about "orders", include 'Orders' and 'OrderItems' if details are needed, or 'Customers' if customer info is required.
    4. If the user asks about "products" or "stock", include 'Products' and 'Inventory'.
    5. Return a JSON object with a "tables" key containing a list of table names.
       Example: {{"tables": ["Orders", "Customers"]}}
    """,
                )
            ]
        )
...
def classify_query_intent(query: str) -> str:
    ...
    2. Is a general question about capabilities, greeting, OR even a request for non-business info (e.g., 'hello', 'write python code', 'solve random math problems') -> 'general_query'
    3. Is a request for DATABASE METADATA or SCHEMA (e.g., 'list tables', 'show schema', 'database structure') -> 'general_query'
...
def handle_general_query(query: str) -> Dict[str, Any]:
    ...
    **TONE & PERSONA:**
    - **Name:** SaasCee
    - **Role:** Business Assistant for SenseSaaS Retail Analytics platform.
    - **Tone:** Professional, helpful, and direct.

    **INSTRUCTIONS:**
    1. If the user greets you or asks what you can do, explain you are here to help visualize and query their BUSINESS data (Orders, Products, Customers, Inventory).
