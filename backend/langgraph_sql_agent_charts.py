...
def get_chart_generation_prompt() -> ChatPromptTemplate:
    ...
    1. **ANALYZE THE REQUEST:**
       ...
       - Determine the best chart type:
         - **Pie:** For parts of a whole (e.g., Order Status Distribution).
         - **Bar:** For categorical comparisons (e.g., Revenue per Product Category).
         - **Line:** For trends over time (e.g., Orders per Day).
    ...
    2. **GENERATE SQL:**
       ...
       - Select human-readable labels (e.g., `Status` enum should be mapped if possible, or select the raw value and we'll map it later).
    ...
    4. **OUTPUT FORMAT:**
       ...
       Example:
       {{
           "sql": "SELECT Status, COUNT(*) as Count FROM Orders WHERE CompanyID = 1 GROUP BY Status",
           "config": {{
